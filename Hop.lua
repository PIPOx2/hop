	...
	function ServerHold:Server(text, img)
		...
		function ChannelHold:Channel(text)
         ...
         function ChannelContent:Toggle(text, default, callback)
				...
				ToggleFrameCorner.CornerRadius = UDim.new(1, 1)
				...
				ToggleFrameCircle.Position = UDim2.new(0.214999999, -5, 0.113000001, 0)
				ToggleFrameCircle.Size = UDim2.new(0, 16, 0, 16)

                ToggleFrameCircleCorner.CornerRadius = UDim.new(0, 8)
                ...
                if default == true then
                    ...
                    TweenService:Create(
						...
						ToggleFrameCircle:TweenPosition(UDim2.new(0.655, -5, 0.113000001, 0), Enum.EasingDirection.Out, Enum.EasingStyle.Quart, .3, true)
                    TweenService:Create(
                        Icon,
                        TweenInfo.new(.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                        {ImageTransparency = 1}
				...
				Toggle.MouseButton1Click:Connect(function()
					...
					elseif  check.loacker ==  true then
						...
						ToggleFrameCircle:TweenPosition(UDim2.new(0.234999999, -5, 0.113000001, 0), Enum.EasingDirection.Out, Enum.EasingStyle.Quart, .3, true)
						TweenService:Create(
							Icon,
			...
			if config == true then
					...
					TweenService:Create(
						ImageButton,
						TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
						{Size = UDim2.new(0, 24, 0, 24)} -- UDim2.new(0, 128, 0, 25)
					):Play()
					check.toogle = true
					callback(check.toogle)
				end
				...
				lockerframe.Size = UDim2.new(0, 400, 0, 35)
				...
				LockerImageLabel.Position = UDim2.new(0.50, 0, 0.5, 0)
...
function ChannelContent:Notification(titel,text,delays)
		...
		pcall(function()
			if io.KeyCode == CloseBind then
				if uitoggled == false then
                    ...
                    MainFrame.Size = UDim2.new(0,0, 0, 0)
                   if minimized == false then
                        MainFrame.Size = UDim2.new(0, 681, 0, 22)                 
                   else
                        MainFrame.Size = UDim2.new(0, 681, 0, 396)
                   end
                   ChannelContent:Notification("UI Toggle","Has Been Close",2.5)
				else
					uitoggled = false
...
btns:Toggle("Auto_Hop",false, function(bool)
    endmddn = bool 
        while endmddn do wait() 
				...
				spawn(function()
                        while task.wait() do
                             pcall(function()
                                ...
                                v2.activeController.timeToNextBlock = -999
                                ...
                                else v2.activeController.timeToNextAttack = -999
                                end
                                v2.activeController.attacking = false
                                v2.activeController.hitboxMagnitude = 200
                                v2.activeController.focusStart = 1655503339.0980349
                                v2.activeController.increment = 4
                                v2.activeController.blocking = false
                                v2.activeController.attacking = false
                                v2.activeController.humanoid.AutoRotate = true
                                V2.activeController.increment = tonumber(getgenv().increment) 
                                if getgenv().increment ~= nil then 
                                round(v2,0); 
